...
namespace DTU.Data.Repositories
{
    ...
    {
        IEnumerable<NhanVien> GetAllByTag(string timKiem, int pageIndex, int pageSize, out int totalRow);
        void KhoaMo(int id);
    ...
    public class NhanVienRepository : RepositoryBase<NhanVien>, INhanVienRepository
    {
        ...
        public IEnumerable<NhanVien> GetAllByTag(string timKiem, int pageIndex, int pageSize, out int totalRow)
        {
            var query = from p in DbContext.NhanViens
                        where p.TenNhanVien.Contains(timKiem) ||
